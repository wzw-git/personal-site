...
| 模块 | 说明 |
|------|------|
| 访客 | 文章列表与筛选、文章详情（Vue SPA）、**登录用户可发表评论**、公开用户资料页 |
...
| 层级 | 技术 |
|------|------|
| 前端 | **Vue 3 + Vue Router + Vite**（首页与文章页）；后台/写作等仍为静态 HTML；Marked、DOMPurify、Google Fonts |
...
# 构建访客端 Vue 产物到 public/（改前端源码后需重新执行）
...
- 文章首页：`/` 或 `/index.html`（由 Vue 构建的 `public/index.html` 提供）
- 后台与登录：`/admin.html`
- 写作：`/write.html`
- 部署与安全长文：`/博客说明.html`
...
├── frontend/             # Vue 3 源码（npm run build → public/）
├── nginx/default.conf    # 静态站点与 /api 反代
├── public/               # 构建后的静态资源与 uploads/（用户上传）
...
cd frontend
npm install
npm run dev   # 默认代理 /api → localhost:3000，见 vite.config.js
```

前端通过 `public/js/api-base.js` 指向本地 API 地址即可联调静态页（注意浏览器 CORS）；Vue 开发服务器使用 Vite 代理即可。
